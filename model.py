...
class TextModel(nn.Module):
    ...
    def __init__(self, vocab, weight_matrix, args, initrange=0.1):
        super().__init__()
        self.vocab = vocab
...
class DAE(TextModel):
    ...
    def __init__(self, vocab, weight_matrix, args):
        super().__init__(vocab, weight_matrix, args)
    ...
    def encode(self, input):
        # input = self.drop(self.embed(input) + torch.Tensor(random_n(300,mean=0, scale=scale, lower=-A, upper=A)).to(device))
        # input = self.drop(self.embed(input) + torch.Tensor(np.random.normal(0, scale, 300)).to(device))
