...
class MultivariateCalibrated:
    def __init__(self, epsilon,
        ...
        if embedding_matrix is not None:
            ...
            self.embedding_matrix = gensim.models.KeyedVectors.load_word2vec_format(embed, binary=False,
...
class TruncatedGumbel:
    def __init__(self,
        ...
        if self.max_inter_dist == 0 or self.min_inter_dist == np.inf:
            ...
            D, _ = index_flat.search(np.array([self.embedding_matrix[start]]).astype(np.float32),
        ...
        if self.a * self.min_inter_dist <= 0:
            ...
            self.b = 2 * self.max_inter_dist / (np.min(
                np.array([lambertw(2 * self.a * self.max_inter_dist).real, np.log(self.a * self.min_inter_dist)])))
        ...
        if self.return_noise == True:
            if privatized_word == "⊥":
                new_word = np.random.choice(
                    [word for word in self.embedding_matrix.key_to_index.keys() if word not in Lw])
                return new_word, noise
            else:
...
class Mahalanobis:
    ...
    def get_perturbed_vector(self, word_vec, n):
        ...
        Z = np.multiply(np.random.gamma(n, 1 / self.epsilon),
                        np.dot(sqrtm(self.lambd * self.cov_mat + (1 - self.lambd) * self.identity_mat), norm_noise))
        return word_vec + Z, Z
...
class SynTF:
    ...
    def get_synonym_score(self, word):
        ...
        for syn in self.syn_dict[word]:
            if syn in self.tfidf_matrix.index:
                score_dict[syn] = self.tfidf_matrix.loc[word][0]
            else:
                score_dict[syn] = 0
        return score_dict
